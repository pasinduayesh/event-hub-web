<?php

session_start();
require_once './includes/db.php';

if (!isset($_SESSION['user_id'])) {
    header("Location: ./login.php");
}

?>
<!DOCTYPE html>
<html lang="en">

    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"
            integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw=="
            crossorigin="anonymous" referrerpolicy="no-referrer" />
        <link rel="icon" type="image/x-icon" href="../resources/images/root/favicon.png">
        <link rel="stylesheet" href="./resources/styles/index.css">
        <title>EventHub | Home</title>

        <script>
            function openAccountMenu(dropDown) {
                dropDown.classList.toggle("hide");
            }
        </script>
    </head>

    <body>

        <div class="background-pattern">
            <img class="blob blob-2" src="./resources/images/root/Component 2.png" alt="" srcset="">
            <img class="blob blob-1" src="./resources/images/root/Component 1.png" alt="" srcset="">
            <div class="pattern"></div>
        </div>

        <nav>

            <div class="nav-wrapper">

                <img src="./resources/images/root/eventhub black.png" alt="Logo">

                <div class="nav-links">

                    <ul>
                        <li onclick="window.location.href='./index.php'">
                            Home
                        </li>
                        <li onclick="window.location.href='./events.php'">
                            Events
                        </li>
                    </ul>
                </div>



                <div class="icon-container">

                    <button class="Create-Event" onclick="window.location.href = './add-event.php'">
                        <i class="svg-wrapper">
                            <svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 448 512"><!--! Font Awesome Pro 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
                                <path
                                    d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" />
                            </svg>
                        </i>

                        <span>Event</span>
                    </button>

                    <?php
                    // PHP code to retrieve the user's image
                    $stmt = $conn->prepare("SELECT img from users WHERE id = ?");
                    $stmt->bind_param('s', $_SESSION['user_id']);
                    $stmt->execute();

                    // Bind the result variables
                    $stmt->bind_result($img);

                    $stmt->fetch();

                    if ($img == '') {
                        $imageUrl = 'https://st3.depositphotos.com/6672868/13701/v/450/depositphotos_137014128-stock-illustration-user-profile-icon.jpg';
                    } else {

                        $imageUrl = "./resources/images/user/" . $img;
                    }

                    $stmt->close();
                    ?>

                    <button
                        style="background-image: url(<?php echo $imageUrl ?>);width: 40px;height: 40px;background-size: cover;background-position: center;border-radius: 100%;border: 1px solid;"
                        class="icons" onclick="openAccountMenu(document.querySelector('.drop-down-menu'))">

                    </button>
                </div>

                <div class="drop-down-menu account hide">
                    <ul>
                        <li onclick="window.location.href='./account.php'">
                            My Account
                        </li>
                        <li onclick="window.location.href='./manage-events.php'">
                            My Events
                        </li>
                        <li onclick="window.location.href='./booked-events.php'">
                            My Bookings
                        </li>
                        <li onclick="window.location.href='./logout.php'">
                            Logout
                        </li>
                    </ul>
                </div>

            </div>


        </nav>